...
27. **Prescrição que o aluno já treinou se arquiva, nunca se apaga.** `removerDia()` e `removerItemDoDia()` conferem se existe sessão ou carga apontando para aquilo e, se existir, gravam `archived_at` em vez de excluir; devolvem `{ arquivado }` para a tela contar o que houve. Não é preciosismo com histórico: apagar a prescrição usada era **recusado pelo banco**. A FK de `exercise_logs.workout_day_exercise_id` é `on delete set null`, e o índice `exercise_logs_uma_por_serie` é `nulls not distinct` — duas cargas da mesma sessão com o mesmo `set_number` viravam a mesma chave assim que os itens saíam juntos, e o professor via `duplicate key value violates unique constraint`. Pior: quando a exclusão passava, o `set null` era um UPDATE, o UPDATE disparava `checar_vinculo_de_presenca` e o `marked_by` da sessão era **regravado** — treino marcado pelo aluno passava a constar como marcado pelo professor. Os dois foram provados contra o Supabase real em 16/09/2026. A saída não é afrouxar o índice (ele impede a mesma série contada duas vezes): é não produzir o nulo. Três triggers `before delete` recusam a exclusão com frase legível caso outro caminho tente. Cópia não herda arquivado, e `normalizar()`/`montarFicha()` escondem arquivado num lugar só.
